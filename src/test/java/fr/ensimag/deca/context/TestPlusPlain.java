...
public class TestPlusPlain {
    ...
    public void testType() throws ContextualError {
        ...
        when(left.verifyExpr(compiler, null, null, null)).thenReturn(INT);
        AbstractExpr right = Mockito.mock(AbstractExpr.class);
        when(right.verifyExpr(compiler, null, null, null)).thenReturn(INT);
        Plus t = new Plus(left, right);
        // check the result
        assertTrue(t.verifyExpr(compiler, null, null, null).isInt());
        // check that the mocks have been called properly.
        verify(left).verifyExpr(compiler, null, null, null);
        verify(right).verifyExpr(compiler, null, null, null);
    }
}